...
class SpatialAttention(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(
            in_channels=2, out_channels=1, kernel_size=7, padding=7 // 2, bias=False
        )
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        self.shortcut = (
            nn.Identity() if in_dim == out_dim else nn.Conv2d(in_dim, out_dim, 1)
        )
...
class TemporalTransformer(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        if self.only_self_att:  # no cross-attention
            ...
            for i, block in enumerate(self.transformer_blocks):
                context[i] = rearrange(
                    context[i], "(b f) l con -> b f l con", f=self.frames
                ).contiguous()
        ...
        self.att_layers = nn.ModuleList(
            [
                TemporalAttentionBlock(
                    dim, heads, dim_head, rotary_emb, use_image_dataset, use_sim_mask
                )
